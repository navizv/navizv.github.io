...
ways-own [price volume curvolume avgcurvolume company]
streams-own [price volume curvolume]

turtles-own [in-use? min-len min-neib tmpvolume name c-num p-o-t]
...
to edit-properties
  ...
  ifelse ver != nobody [][
    ...
    ifelse ver != nobody [
      ask ver [
        set price read-from-string user-input "Введите цену для данной дороги"
        set label (word price "; " volume "; " company)
        set label-color black
      ]
    ][
      ...
      if ver != nobody [
        ask ver [
          set price read-from-string user-input "Введите цену для данного потока"
...
to edit-properties-v
  ...
  ifelse ver != nobody [][
    ...
    ifelse ver != nobody [
      ask ver [
        set volume read-from-string user-input "Введите объём для данной дороги"
        set label (word price "; " volume "; " company)
        set label-color black
      ]
    ][
      ...
      if ver != nobody [
        ask ver [
          set volume read-from-string user-input "Введите объём для данного потока"
          set label (word price "; " volume)
          set label-color black
        ]
      ]
...
to edit-company
  ...
  ifelse ver != nobody [][
    ...
    if ver != nobody [
      ask ver [
        set company read-from-string user-input "Введите номер компании для данной дороги"
        set label (word price "; " volume "; " company)
        set label-color black
      ]
    ]
...
to edit-name
  ...
  if ver != nobody [
    ask ver [
      set name user-input "Введите название"
      set label name
      set label-color black
    ]
  ]
...
to draw-ways
  ...
  ask ways [
    set label (word price "; " volume "; " curvolume "; " company)
...
to-report price-of-to [ver]
  ...
  let cm 0
  if c-num != 0 and [c-num] of min-neib != c-num [
    set cm [price] of way-with min-neib
  ]
  ask way-with ver [
    if tmp != 1000000 [
      ifelse company != 0 and curcompany = company [
        set tmp tmp - cm + ( cm + price ) * ( 1 - skidka ) ; СКИДКА!
      ][
        set tmp tmp + price
      ]
    ]
...
88
загрузить мир
import-world \"default.csv\";user-file;
NIL
1
T
OBSERVER
