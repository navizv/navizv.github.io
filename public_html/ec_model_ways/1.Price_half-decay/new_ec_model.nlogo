...
ways-own [price]
streams-own [volume]
turtles-own [in-use? min-len min-neib]
globals [nt]
...
to delete-object
  ...
  ifelse ver != nobody [
    ...
    set ver min-one-of ways with [distancexy mouse-xcor mouse-ycor < 2] [distancexy mouse-xcor mouse-ycor]
    ifelse ver != nobody [
      ask ver [die]
    ][
      set ver min-one-of streams with [distancexy mouse-xcor mouse-ycor < 2] [distancexy mouse-xcor mouse-ycor]
      if ver != nobody [
        ask ver [die]
        ;!!!!
      ]
...
to edit-properties
  ...
  ifelse ver != nobody [][
    ...
    ifelse ver != nobody [
      ask ver [
        set price read-from-string user-input "Введите цену для данного пути"
        set label price
        set label-color black
      ]
    ][
...
to find-min-way-to [dest]
  ...
  set in-use? true
  let possible way-neighbors with [not in-use?]
...
55
фон
import-drawing \"ris.gif\";user-file
NIL
1
T
OBSERVER
